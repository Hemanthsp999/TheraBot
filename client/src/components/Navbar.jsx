    ...
    <nav className="navbar fixed top-0 left-0 w-full bg-black text-white p-4 shadow-md">
      <div className="container mx-auto flex justify-between items-center px-4 md:px-8">
        ...
        <div className="flex items-center space-x-2">
          <img src={Bot} className="w-8 h-8 md:w-10 md:h-10 rounded-full object-cover" alt="Logo"/>
          <a href="#" className="text-lg md:text-xl font-bold whitespace-nowrap">Therabot</a>
        </div>
        ...
        <ul className="hidden md:flex gap-6">
          {["Home", "About", "Services", "Contact"].map((item, index) => (
            <li key={index} className="py-2">
              <a href="#" className="hover:text-blue-500 transition">{item}</a>
            </li>
          ))}
        </ul>
