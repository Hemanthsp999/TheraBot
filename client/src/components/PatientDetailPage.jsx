    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            setPatient({
                ...
                condition: 'Anxiety',
                status: 'Active',
                notes: 'Patient has been showing improvement with current therapy approach.',
                therapyHistory: [
                    { date: '2023-11-15', type: 'CBT Session', notes: 'Discussed anxiety triggers and coping mechanisms.' },
                    { date: '2023-11-01', type: 'Initial Assessment', notes: 'Patient reported anxiety symptoms for past 6 months.' },
                    { date: '2023-10-15', type: 'CBT Session', notes: 'Introduced mindfulness techniques.' }
                ],
                medications: [
                    { name: 'Sertraline', dosage: '50mg', frequency: 'Daily', startDate: '2023-10-01' }
                ]
            });
            setLoading(false);
        }, 800);
                    ...
                    <div className="p-6">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                            ...
                            <div className="mt-4 md:mt-0">
                                <span className={`px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full ${patient.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                    }`}>
                                    {patient.status}
                                </span>
                            </div>
                        ...
                        <div className="border-b border-gray-200 mb-6">
                            <nav className="-mb-px flex space-x-8">
                                ...
                                <button
                                    className={`py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'therapy'
                                        ? 'border-blue-500 text-blue-600'
                                        : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                                        }`}
                                ...
                                <button
                                    className={`py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'medications'
                                        ? 'border-blue-500 text-blue-600'
                                        : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                                        }`}
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {patient.therapyHistory.map((session, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td className="px-6 py-4 text-sm text-gray-500">{session.notes}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
