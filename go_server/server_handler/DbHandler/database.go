...
func AddUser(user_model datahandler.User, db *sql.DB) error {
	bytes, err := bcrypt.GenerateFromPassword([]byte(user_model.UserPass), bcrypt.DefaultCost)
	if err != nil {
		return err
	}

	query := `INSERT INTO users (name, email, password_hash, role) VALUES (?, ?, ?, ?)`
	_, err = db.Exec(query, user_model.UserName, user_model.UserEmail, string(bytes), user_model.UserRole)
	return err
}

func IsValidUser(user_model datahandler.User, db *sql.DB) bool {
	var storedHash string
	err := db.QueryRow("SELECT password_hash FROM users WHERE email = ?", user_model.UserEmail).Scan(&storedHash)
